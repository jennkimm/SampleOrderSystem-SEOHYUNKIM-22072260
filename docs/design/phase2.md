...
- [x] 주문 접수 후 RESERVED 목록에 표시되는가
- [x] 재고가 충분한 시료를 승인하면 CONFIRMED로 전환되는가
- [x] 재고가 부족한 시료를 승인하면 PRODUCING으로 전환되는가
- [x] 거절 처리 후 해당 주문이 RESERVED 목록에서 사라지는가
- [x] 존재하지 않는 시료 ID로 주문 시 안내 메시지가 출력되는가
