...
- [x] 재고 부족 주문 승인 시 생산 라인에 자동 등록되는가
- [x] 생산 큐가 FIFO 순서로 표시되는가
- [x] `scheduledQty = ceil(부족분 / (수율 × 0.9))`대로 올림 처리되는가
- [x] 생산 완료 후 주문이 CONFIRMED로 전환되는가
- [x] 생산 완료 후 해당 시료의 재고가 `scheduledQty`만큼 증가하는가
- [x] 생산 완료 → 출고(Phase 3)까지 이어지는 전체 흐름이 동작하는가
